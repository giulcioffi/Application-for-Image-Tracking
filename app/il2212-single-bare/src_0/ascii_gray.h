 ...
 */
void printAsciiHidden(unsigned char* image, int x_dim, int y_dim, int x_pos, int y_pos, int size, unsigned char gray_value) {
  int k = 0;
  int l = 0;
  unsigned char c_pixel, pixel;
  for(k = 0; k < y_dim; k++)
  {
    for(l = 0; l < x_dim; l++) {	
      if ((k >= y_pos - size) && (k <= y_pos + size) && (l >= x_pos - size) && (l <= x_pos + size)) {
    	 ...
    	 // printf("%4c", asciiChars[((NR_ASCII_CHARS - 1) * c_pixel) / 255]);
    	  ...
    	  pixel = image[k * y_dim + l];

      }
      c_pixel = pixel > 255 ? 255 : pixel;
      printf("%4c", asciiChars[((NR_ASCII_CHARS - 1) * c_pixel) / 255]);
    }
    printf("\n");
  }
